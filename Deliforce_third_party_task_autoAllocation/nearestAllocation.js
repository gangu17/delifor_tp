...
function nearByAllo(event, cb,dynamicTone) {
  ...
  if (driverArry.length) {
    ...
    taskModel.findOne({_id: mongoose.Types.ObjectId(taskId)}).then((taskData) => {
      if (!_.isEmpty(taskData)) {
        ...
        Promise.all([assignNearestDriverForcefully(taskId, driverArry[0].driverId), notification.sendPushNotification(taskData, driverArry[0].endArn, 5, expiry,null,null,null,dynamicTone)])
          .then(() => {
