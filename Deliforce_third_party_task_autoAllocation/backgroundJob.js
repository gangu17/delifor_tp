...
module.exports = {
  doBackGroundJob: (event, cb,dynamicTone) => {
    ...
    fetchTaskDetails(taskId).then((taskDetails) => {
      ...
      if (!_.isEmpty(taskDetails)) {
        return sendPushNotifications(driverArry, remaingArry, taskId, taskDetails, expiry, cb,dynamicTone);
      } else {
...
function sendPushNotifications(driverArry, remaingArry, taskId, taskDetails, timeout, cb,dynamicTone) {
  let expireTime = Number(timeout) * 1000;

  function AsyncFunction(driverDetails) {
    console.log('async', JSON.stringify(driverDetails));

    return notification.sendPushNotification(taskDetails, driverDetails.endArn, 4, timeout,null,null,null,dynamicTone);
