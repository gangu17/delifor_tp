...
function sendAll(event, cb,dynamicTone) {
  ...
  if (driverArry.length) {
    ...
    taskModel.findOne({_id: mongoose.Types.ObjectId(taskId)}).then((taskData) => {
      for (let i = 0; i < driverArry.length; i++) {
        ...
        promiseArry.push(sendNotifications(taskData, driverArry[i].endArn, 4, expiry, i, driverArry.length - 1, cb,dynamicTone));
      }
...
function sendNotifications(taskDetails, endpointArn, flag, expiry, i, driverLength, cb,dynamicTone) {
  ...
  console.log('i' + i);
  console.log('driverLength' + driverLength);
  let expiryConvert = expiry * 1000;
  return notification.sendPushNotification(taskDetails, endpointArn, 4, expiry,null,null,null,dynamicTone).then(() => {
    console.log('push sent');

    if (i === driverLength) {
      return  new Promise((reslove,reject)=>{
        setTimeout(function(){return taskModel.findOne({_id: mongoose.Types.ObjectId(taskDetails._id)}).then((taskIn) => {
